...
(defun fft-test (n m &key (part :real) (scale 0.1) (task-level-limit 1) (debug nil))
  (let ((*debug* debug))
    (graphit (funcall
	      (case part
		(:mag #'vmag)
		(:real #'vreal)
		(:imag #'vimag))
	      (fft (f n m) task-level-limit))
	     scale)))

(defvar *future-level*)

(proclaim '(fixnum *future-level*))

(defun fft (x &optional (task-level-limit 0))
  (let ((*future-level* (* (truncate (length x) task-level-limit)
			   2)))
    (fft1 x)))
  ...
  (let ((N (length x)))
	...
	(multiple-value-bind
	 ...
	 (combine-ffts
	  (if (>= N *future-level*)
	      (progn
		(if *debug* (atomic-inform-user N 'even))
		(future (fft1 even)))
	      (fft1 even))
	  (if (>= N *future-level*)
	      (progn
		(if *debug* (atomic-inform-user N 'odd))
		(future (fft1 odd)))
	      (fft1 odd)))))))
